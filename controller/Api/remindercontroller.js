  ...
  reminder_list: async (req, res) => {
    try {
      ...
      const reminderList = reminderData.map(reminder => {
        ...
        // Calculate remaining time from the current time until the reminder_start_time
        const currentTime = new Date();
        const remainingTimeInMillis = reminder_start_time.getTime() - currentTime.getTime();
        const remainingMinutes = Math.ceil(remainingTimeInMillis / (1000 * 60));
