...
cron.schedule('*/5 * * * * *', async () => {
  ...
  const reminderCount = await reminderModel.count({ upcoming_time: formattedCurrentDateTime });
  if (reminderCount) {
    await reminderModel
      ...
      for (let data of reminderData) {
        ...
        const updatedTiming =  formattedCurrentDateTime + hour;

        await reminderModel.findByIdAndUpdate(
          { _id: data._id },
          { upcoming_time: updatedTiming }
          );
    await helper.send_push_notifications(data);
    }
  }
});
